...
class CrescendoResponse(BaseModel):
    generated_question: str = Field(description="The next question in the conversation")
    last_response_summary: str = Field(default="", description="Summary of the last response")
    rationale_behind_jailbreak: str = Field(default="", description="Rationale behind the jailbreak attempt")
...
def _fallback_response(retry_state: RetryCallState):
    ...
    message: Optional[Message] = retry_state.kwargs.get("message")
    if not message:
        return []
...
class AzureRAISimulationTarget(PromptChatTarget):
    ...
    def _extract_json_from_content(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        # Attempt 2: Remove markdown blocks
        # Look for ```json ... ``` or just ``` ... ```
        markdown_pattern = re.compile(r"```(?:json)?\s*(.*?)\s*```", re.DOTALL)
        match = markdown_pattern.search(content)
        if match:
        ...
        # This handles cases where there is text before/after the JSON but no markdown
        start = content.find("{")
        end = content.rfind("}")
            ...
            if isinstance(response, dict):
                    ...
                    if "output" in result and isinstance(result["output"], dict):
                        ...
                        if "choices" in output and len(output["choices"]) > 0:
                            ...
                            if "message" in choice and "content" in choice["message"]:
                                ...
                                parsed = self._extract_json_from_content(content_str)
                                return parsed if parsed is not None else {"content": content_str}
                ...
                if "content" in response:
                    ...
                    if isinstance(content_str, str):
                        parsed = self._extract_json_from_content(content_str)
                        return parsed if parsed is not None else {"content": content_str}
                    return content_str if isinstance(content_str, dict) else {"content": str(content_str)}
    ...
    async def send_prompt_async(
        ...
        :return: The response
        """
        self.logger.info("Starting send_prompt_async operation in AzureRAISimulationTarget")
        ...
        try:
            ...
            # Ensure required fields exist for Crescendo
            if self.crescendo_format:
                # Try to parse content if it's a wrapper
                if "content" in response_text and isinstance(response_text["content"], str):
                    extracted = self._extract_json_from_content(response_text["content"])
                    if extracted:
                        response_text = extracted

                try:
                    validated = CrescendoResponse.model_validate(response_text)
                    response_text = validated.model_dump()
                except ValidationError as e:
                    # Try to extract from content if validation failed
                    content_value = response_text.get("content", "")
                    if isinstance(content_value, str):
                        try:
                            # Try to find JSON block
                            json_match = re.search(r'\{.*"generated_question".*\}', content_value, re.DOTALL)
                            if json_match:
                                ...
                                try:
                                    ...
                                    # Try fixing quotes if direct parse fails
                                    fixed_json = json_str.replace("'", '"')
                                    parsed_content = json.loads(fixed_json)
                                
                                validated = CrescendoResponse.model_validate(parsed_content)
                                response_text = validated.model_dump()
                            else:
                                # Try to find just the generated_question field
                                question_match = re.search(r'"generated_question"\s*:\s*"(.*?)"', content_value, re.DOTALL)
                                if question_match:
                                    response_text = {
                                        "generated_question": question_match.group(1),
                                        "last_response_summary": "",
                                        "rationale_behind_jailbreak": ""
                                    }
                                else:
                                    raise ValueError(f"Response missing 'generated_question' field: {e}")
                        except Exception as inner_e:
                            raise ValueError(f"Response validation failed and extraction failed: {inner_e}")
                    else:
