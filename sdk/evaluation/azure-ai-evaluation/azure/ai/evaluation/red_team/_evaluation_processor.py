...
class EvaluationProcessor:
    ...
    async def evaluate_conversation(
        ...
        context = [
            msg["context"]
            for msg in messages
            if msg.get("role") == "user" and "context" in msg
        ]
        tool_calls = [
            msg.get("tool_calls", [])
            for msg in messages
        ...
        if assistant_messages:
            ...
            try:
                ...
                if hasattr(evaluate_output, "results") or (
                    ...
                    if eval_result:
                        # Check for soft evaluation error (200 OK but outcome=error)
                        eval_properties = eval_result.get("properties", {})
                        if (
                            isinstance(eval_properties, dict)
                            and eval_properties.get("outcome") == "error"
                        ):
                            error_detail = eval_properties.get(
                                "error", "Unknown evaluation error"
                            )
                            self.logger.warning(
                                f"Evaluation returned soft error for conversation {idx+1} "
                                f"{risk_category.value}/{strategy_name}: {error_detail}"
                            )
                            # Return error row — the retry decorator already handles network retries,
                            # and the soft error is captured in the output item for honest reporting
                            return {
                                "inputs.conversation": {"messages": messages},
                                "error": f"Evaluation soft error: {error_detail}",
                            }
