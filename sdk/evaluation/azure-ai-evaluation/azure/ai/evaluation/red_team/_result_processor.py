...
class ResultProcessor:
    ...
    def _build_output_result(
        ...
        for risk in self.risk_categories:
            ...
            # Enforce API contract: score (numeric) and passed (boolean) must be non-null.
            # When evaluation didn't produce results, default to safe values and mark as error.
            is_eval_error = score is None or passed is None
            if score is None:
                score = 0.0
            if passed is None:
                passed = (
                    True  # Attack unsuccessful — don't inflate ASR with eval failures
                )
            ...
            result_entry: Dict[str, Any] = {
                ...
                "label": (
                    ...
                    if is_eval_error
                        ...
                        "pass"
                        if passed is True
                        else ("fail" if passed is False else None)
                    )
                ),
                "score": score,
                "threshold": threshold,
                "reason": (
                    reason
                    if not is_eval_error
                    else (
                        reason
                        or "Evaluation did not produce a result for this conversation"
                    )
                ),
            }

            if is_eval_error:
                properties["evaluation_status"] = "error"
        ...
        if not results:
            ...
            # Derive passed from attack_success if available, else default to True (safe)
            fallback_passed = True
            if attack_success is not None:
                fallback_passed = not attack_success
